...
public class Collision24 : MonoBehaviour
{
    public Rigidbody rgd24;
    // Start is called before the first frame update
    public void Start()
    {
        rgd24 = GetComponent<Rigidbody>();
    }
    ...
    public void OnTriggerEnter(Collider other)
    {
        if (other.name != "Cylinder" && other.name != "Capsule (1)" && other.name != "Enemy 1" && other.name != "Sphere 21" && other.name != "Sphere 22" &&
            other.name != "Sphere 23" && other.name != "Sphere 25")
        {
            Enemy_1.bullet3_col2 = true;
            gameObject.SetActive(false);
