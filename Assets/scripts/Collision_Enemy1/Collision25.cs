...
public class Collision25 : MonoBehaviour
{
    public Rigidbody rgd25;
    // Start is called before the first frame update
    public void Start()
    {
        rgd25 = GetComponent<Rigidbody>();
    }
    ...
    public void OnTriggerEnter(Collider other)
    {
        if (other.name != "Cylinder" && other.name != "Capsule (1)" && other.name != "Enemy 1" && other.name != "Sphere 21" && other.name != "Sphere 22" &&
            other.name != "Sphere 23" && other.name != "Sphere 24")
        {
            Enemy_1.bullet4_col2 = true;
            gameObject.SetActive(false);
